...
////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////
public class FlutterMidiCommandPlusPlugin: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, FlutterPlugin {
    // MIDI
    var midiClient = MIDIClientRef()
    var connectedDevices: [String : ConnectedDevice] = Dictionary<String, ConnectedDevice>()
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "sendData":
            if let packet = call.arguments as? Dictionary<String, Any> {
                sendData(packet["data"] as! FlutterStandardTypedData, deviceId: packet["deviceId"] as? String, portId: packet["portId"] as? UInt64, timestamp: packet["timestamp"] as? UInt64)
                result(nil)
            } else {
    ...
    func sendData(_ data:FlutterStandardTypedData, deviceId: String?, portId: UInt64?, timestamp: UInt64?) {
    ...
    override func send(bytes: [UInt8], timestamp: UInt64?, portId: UInt64?) {
        ...
        if let ep = outEndpoint {
            splitDataIntoMIDIPackets(bytes: bytes, timestamp: timestamp) { packetListPointer in
                MIDISend(portId ?? outputPort, ep, packetListPointer)
            }
        } else {
