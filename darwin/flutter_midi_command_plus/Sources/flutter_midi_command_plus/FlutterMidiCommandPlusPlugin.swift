...
func displayName(endpoint: MIDIEndpointRef) -> String {
    return FlutterMidiCommandPlusPlugin.getMIDIProperty(
...
////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////
public class FlutterMidiCommandPlusPlugin: NSObject, CBCentralManagerDelegate,
    CBPeripheralDelegate, FlutterPlugin
{
    // MIDI
    var midiClient = MIDIClientRef()
    var connectedDevices: [String: ConnectedDevice] = [
    ...
    let midiLog = OSLog(
        subsystem: "com.invisiblewrench.FlutterMidiCommand",
        category: "MIDI"
    )
        ...
        case "sendData":
            if let packet = call.arguments as? [String: Any] {
                sendData(
                    ...
                    deviceId: packet["deviceId"] as? String,
                    portId: packet["portId"] as? UInt64,
    ...
    func connectToDevice(deviceId: String, type: String, ports: [Port]?) {
        ...
        if type == "BLE" {
            if let periph = discoveredDevices.filter({ (p) -> Bool in
                ...
                let device = ConnectedBLEDevice(
                    ...
                    type: type,
                    streamHandler: rxStreamHandler,
                    result: ongoingConnections[deviceId],
                    peripheral: periph,
                    ports: ports
                )
        ...
        {
            let device =
                (type == "native" || type == "network")
                ? ConnectedNativeDevice(
                    id: deviceId,
                    type: type,
                    streamHandler: rxStreamHandler,
                    client: midiClient,
    ...
    func sendData(
        ...
        if let deviceId = deviceId {
            if let device: ConnectedDevice = connectedDevices[deviceId] {
                device.send(bytes: bytes, timestamp: timestamp, portId: portId)
            }
        } else {
    ...
    static func getMIDIProperty(_ prop: CFString, fromObject obj: MIDIObjectRef)
        -> String
    {
        ...
        if err == OSStatus(noErr) {
            result = param!.takeRetainedValue() as String
        }
        return result
    }
    ...
    func createPortDict(count: Int) -> [[String: Any]] {
        return (0..<count).map { (id) -> [String: Any] in
...
class ConnectedDevice: NSObject {
    ...
    func send(bytes: [UInt8], timestamp: UInt64?, portId: UInt64?) {}
    ...
    override func handlePacketList(
        ...
        if #available(macOS 10.15, iOS 13.0, *) {
            ...
            for _ in 0..<packets.numPackets {
                ...
                let data = Data(bytes: &tmp, count: Int(p.length))
                let timestamp = UInt64(
                    round(Double(p.timeStamp) * timestampFactor)
                )
                parseData(data: data, timestamp: timestamp)
                ap = MIDIPacketNext(ap)
            }
